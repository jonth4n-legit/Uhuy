...
class FirefoxRelayService:
    ...
    def create_relay_mask(self, description: str = None) -> Optional[Dict]:
        ...
        try:
            ...
            return {
                'id': mask_data.get('id'),
                'address': mask_data.get('address'),
                'domain': mask_data.get('domain'),
                'full_address': mask_data.get('full_address'),
                'description': mask_data.get('description'),
                'enabled': mask_data.get('enabled', True),
                'created_at': mask_data.get('created_at')
            }
        ...
        try:
            ...
            for mask in mask_list:
                masks.append({
                    ...
                    'domain': mask.get('domain'),
                    'full_address': mask.get('full_address'),
                    'description': mask.get('description'),
                    'enabled': mask.get('enabled', True),
                    'created_at': mask.get('created_at'),
                    'num_forwarded': mask.get('num_forwarded', 0),
                    'num_blocked': mask.get('num_blocked', 0)
                })
    ...
    def test_connection(self) -> Dict[str, Any]:
        ...
        base_urls = [
            ...
            ('Development', 'https://dev.fxprivaterelay.nonprod.cloudops.mozgcp.net')
        ]
    ...
    def get_relay_mask_details(self, mask_id: str) -> Optional[Dict]:
        ...
        try:
            ...
            return {
                ...
                'enabled': mask_data.get('enabled', True),
                'created_at': mask_data.get('created_at'),
                'num_forwarded': mask_data.get('num_forwarded', 0),
                'num_blocked': mask_data.get('num_blocked', 0)
            }
        except requests.RequestException as e:
