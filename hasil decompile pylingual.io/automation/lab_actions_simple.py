...
async def start_lab(page: Page, logger, handle_captcha: Callable[[], Awaitable[bool]], 
                   lab_url: str, wait_networkidle: bool = True) -> dict:
    """Buka halaman lab, klik Start Lab natural, lalu tangani reCAPTCHA jika terlihat."""
    ...
    start_button_texts = [
        'Mulai Lab', 'Start Lab', 'Mulai lab', 'Start lab', 'MULAI LAB', 'START LAB',
        'Begin Lab', 'Mulai', 'Start', 'Begin', 'Mulai Lab, dan Anda memiliki',
    ...
    # Try to scroll to control panel
    try:
    ...
    # Method 2: Find anchor frame manually
    ...
    # Method 3: Fallback search all frames
    if not clicked_checkbox:
        try:
            for fr in page.frames:
                try:
...
async def open_cloud_console(page: Page, logger, browser: Browser, timeout_sec: int = 400) -> Tuple[dict, Optional[BrowserContext], Optional[Page]]:
    ...
    while asyncio.get_event_loop().time() < deadline:
        ...
        try:
            ...
            for i in range(min(total, 20)):  # Limit to avoid infinite loops
                cand = btns.nth(i)
                try:
...
async def handle_gcloud_terms(inc_page: Optional[Page], logger, timeout_sec: int = 120) -> dict:
    """Menangani halaman persetujuan Google Cloud (Terms of Service)."""
    if not inc_page:
    ...
    while asyncio.get_event_loop().time() < deadline:
        ...
        if 'accounts.google.com/speedbump' in current_url:
            ...
            for _ in range(5):
                ...
                # Try direct search if iframe didn't work
                ...
                if not checkbox_checked:
                    ...
                    # Find agree button
                    agree_texts = [
                        'Agree and continue', 'Agree & Continue', 'Setuju dan lanjutkan', 
                        'Setuju & Lanjutkan', 'Continue', 'Lanjutkan', 'Accept'
                    ]
...
async def enable_genai_and_create_api_key(inc_page: Page, logger, project_id: str, timeout_sec: int = 180) -> dict:
    ...
    try:
        ...
        # Wait for modal and extract API key
        try:
