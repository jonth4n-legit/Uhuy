...
from automation.lab_actions_simple import (
    start_lab as start_lab_action, 
    open_cloud_console as open_console_action, 
    handle_gcloud_terms as handle_terms_action, 
    enable_genai_and_create_api_key as enable_key_action
)
...
class CloudSkillAutomation:
    ...
    async def _bind_single_page_policy(self):
        ...
        try:
            ...
            # Add script to prevent new tabs
    ...
    async def _start_lab_async(self, lab_url: str) -> Dict:
        ...
        try:
            ...
            result = await start_lab_action(
                page=self.page, 
                logger=self.logger, 
                handle_captcha=lambda: self._handle_captcha(), 
                lab_url=lab_url, 
            ...
            try:
                ...
                console_info, inc_ctx, inc_page = await open_console_action(
                    self.page, self.logger, self.browser, timeout_sec=300
                )
                ...
                result_out = {
                    ...
                    'console_opened': True, 
                    'console_url': inc_page.url if hasattr(inc_page, 'url') else '', 
                    'console_incognito': True, 
    ...
    async def _init_browser(self):
        ...
        # Try extension mode if available
    ...
    async def _fill_registration_form(self, user_data: Dict):
        ...
        try:
            # JavaScript untuk mengisi field dengan deep search
            fill_input_script = '''
            ...
            form_success['first_name'] = await smart_fill_field(
                labels=['First name'], 
                placeholders=['First name'], 
                selectors=first_name_selectors, 
            ...
            form_success['last_name'] = await smart_fill_field(
                labels=['Last name'], 
                placeholders=['Last name'], 
                selectors=last_name_selectors, 
            ...
            form_success['email'] = await smart_fill_field(
                labels=['Email'], 
                placeholders=['Email'], 
            ...
            form_success['company'] = await smart_fill_field(
                labels=['Company'], 
            ...
            form_success['password'] = await smart_fill_field(
                labels=['Password'], 
            ...
            form_success['password_confirm'] = await smart_fill_field(
                labels=['Password confirmation'], 
                placeholders=['Password confirmation'], 
                selectors=confirm_selectors, 
                value=user_data['password_confirm'], 
            ...
            # Validation
            failed_fields = [field for field, success in form_success.items() if not success]
            successful_fields = [field for field, success in form_success.items() if success]
            log_automation_step(self.logger, 'FORM_VALIDATION', 'SUMMARY', {
                'successful_fields': successful_fields, 
                'failed_fields': failed_fields, 
                'success_rate': f'{len(successful_fields)}/{len(form_success)}'
            })
    ...
    async def _click_recaptcha_checkbox(self) -> bool:
        ...
        anchor_frame = None
        for _ in range(3):
            anchor_frame, _ = await self._find_recaptcha_frames()
            if anchor_frame:
                break
    ...
    async def _handle_captcha(self) -> bool:
        ...
        # Extension mode - wait for manual solve
        if self.extension_mode:
            ...
            # Auto solve with captcha service
        ...
        try:
            ...
            while attempt < max_attempts:
                ...
                if audio_src and self.captcha_solver:
                    ...
                    # Try to capture audio via network
                    try:
                    ...
                    if result:
                        ...
                        reload_selectors = [
                            ...
                            'button:has-text(\'Get a new challenge\')'
                        ]
